...
from my_package.core import generate_random_obstacle
...
class RewardExplorerEnv(gym.Env):
    ...
    def reward_check(self) -> int:
        """
        Verifica e raccoglie le ricompense non ancora raccolte dall'agente.
        Questa funzione scorre tutte le ricompense disponibili e controlla se 
        sono all'interno del campo visivo (FoV) dell'agente. Se una ricompensa 
        è visibile e non è stata ancora raccolta, viene segnata come raccolta 
        e il contatore delle ricompense raccolte viene incrementato.
        Returns:
            int: Il numero di ricompense raccolte durante questa verifica.
        """
    ...
    def get_reward(self) -> Tuple[float, bool]:
        """
        Calcola e restituisce il reward corrente e lo stato di terminazione.
        Il metodo valuta se l'agente ha raccolto un reward, se ha raggiunto l'area target
        o se ha superato i confini dell'area di lavoro. Aggiorna lo stato dell'agente e
        modifica il valore del reward di conseguenza.
        Returns:
            Tuple[float, bool]: Una tupla contenente il valore del reward e un booleano
            che indica se l'episodio è terminato.
        """
