...
class ShipUniContEnv(gym.Env):
    """ 
    Copia di ShipRovCont-v0 con vincono dell'uniciclo
    - stato ridotto (non include v_sway)
    - azioni ridotte (non includono v_sway)
    ...
    def get_obs(self) -> Tuple[np.ndarray, Set[int]]:
        ...
        observation[4]      = norm_omega                # Velocità angolare
    ...
    def action_to_controls(self, action: np.ndarray) -> Dict:
        ...
        v_sway  = 0.0
        yaw_rel = action[1] * self.yaw_rel_max
...
if __name__ == "__main__":
    ...
    env = gym.make("ShipUniCont-v2", render_mode="human", Options=Options, workspace=(0, 0, 20, 20))
    low = env.action_low
